 ...
 * across Footwear, Apparel, Sarees, Electronics, Smartphones, Audio, Kitchen, and Watches.
 */
...
export function getRealProductImageFallback(title?: string, category?: string, index: number = 0): string {
  ...
  if (query.includes('shoe') || query.includes('sneaker') || query.includes('puma') || query.includes('nike') || query.includes('adidas') || query.includes('boot') || query.includes('footwear')) {
    const list = CATEGORY_IMAGES.shoes;
    return list[index % list.length];
  }
  if (query.includes('saree') || query.includes('sari') || query.includes('zari') || query.includes('pallu') || query.includes('kanjivaram') || query.includes('banarasi') || query.includes('silk')) {
    const list = CATEGORY_IMAGES.saree;
    return list[index % list.length];
  }
  if (query.includes('kurta') || query.includes('kurti') || query.includes('ethnic')) {
  ...
  if (query.includes('earbud') || query.includes('airpod') || query.includes('tws') || query.includes('airdopes')) {
    const list = CATEGORY_IMAGES.earbuds;
    return list[index % list.length];
  }
  if (query.includes('headphone') || query.includes('headset') || query.includes('audio') || query.includes('speaker')) {
    const list = CATEGORY_IMAGES.headphones;
    return list[index % list.length];
  }
  if (query.includes('watch') || query.includes('smartwatch') || query.includes('clock') || query.includes('band')) {
    const list = CATEGORY_IMAGES.watch;
    return list[index % list.length];
  }
  if (query.includes('phone') || query.includes('mobile') || query.includes('iphone') || query.includes('samsung') || query.includes('galaxy') || query.includes('pixel') || query.includes('oneplus')) {
    const list = CATEGORY_IMAGES.phone;
    return list[index % list.length];
  }
  if (query.includes('laptop') || query.includes('macbook') || query.includes('notebook') || query.includes('computer') || query.includes('pc')) {
    const list = CATEGORY_IMAGES.laptop;
    return list[index % list.length];
  }
  if (query.includes('fryer') || query.includes('cooker') || query.includes('blender') || query.includes('kettle') || query.includes('kitchen') || query.includes('pan')) {
