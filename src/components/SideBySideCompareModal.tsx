...
export const SideBySideCompareModal: React.FC<SideBySideCompareModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/60 dark:bg-black/80 backdrop-blur-sm overflow-y-auto animate-in fade-in duration-200">
      <div className="relative w-full max-w-6xl max-h-[92vh] flex flex-col bg-white dark:bg-[#0d0d0d] rounded-xl shadow-2xl border border-zinc-200 dark:border-[#262626] overflow-hidden text-zinc-900 dark:text-[#f4f4f4] my-auto transition-colors">
        ...
        <div className="sticky top-0 z-10 flex items-center justify-between px-5 py-3.5 border-b border-zinc-200 dark:border-[#242424] bg-white/95 dark:bg-[#0d0d0d]/95 backdrop-blur-md font-mono">
          ...
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={onClearAll}
              className="flex items-center gap-1 px-2.5 py-1 text-xs font-bold uppercase tracking-wider text-rose-600 dark:text-rose-400 hover:bg-rose-50 dark:hover:bg-[#201010] rounded-sm transition-colors cursor-pointer"
            >
        ...
        {products.length >= 2 && (
          <div className="p-4 bg-zinc-50 dark:bg-[#141414] border-b border-zinc-200 dark:border-[#262626] flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-xs font-mono">
        ...
        <div className="p-5 overflow-x-auto">
          <div className="min-w-[750px]">
            <div className="grid grid-cols-1 divide-y divide-zinc-200 dark:divide-[#222222]">
              ...
              <div className={`grid grid-cols-${products.length} gap-4 pb-4`} style={{ gridTemplateColumns: `repeat(${products.length}, minmax(0, 1fr))` }}>
                {products.map((p) => (
                  <div key={p.id} className="relative space-y-2">
                    ...
                    <div className="w-full h-44 rounded-lg overflow-hidden bg-zinc-100 dark:bg-[#141414] border border-zinc-200 dark:border-[#282828]">
                      <img
                        src={p.primaryImage}
                    ...
                    <div className="font-bold text-xs line-clamp-2 text-zinc-900 dark:text-white tracking-tight">
                      {p.canonicalTitle}
                    </div>
                  </div>
                ))}
              ...
              <div className="grid gap-4 py-3 text-xs font-mono" style={{ gridTemplateColumns: `repeat(${products.length}, minmax(0, 1fr))` }}>
                {products.map((p) => (
                  <div key={p.id}>
                    ...
                    <div className="text-base font-black text-emerald-600 dark:text-emerald-400 font-mono-num">
                      ₹{p.minPrice.toLocaleString('en-IN')}
                    </div>
                    <span className="text-[10px] text-zinc-500 dark:text-zinc-400">ON {p.lowestPriceStore.toUpperCase()}</span>
                  </div>
                ))}
              </div>
                      ...
                      {p.listings.map((l, i) => {
                        ...
                        return (
                          <div key={i} className={`flex items-center justify-between p-1.5 rounded text-[11px] border ${
                            isLowest
                              ? 'bg-emerald-50 dark:bg-[#0f1f14] border-emerald-200 dark:border-[#1d4c26] text-zinc-900 dark:text-white'
                              : 'bg-zinc-50 dark:bg-[#151515] border-zinc-200 dark:border-[#222222] text-zinc-700 dark:text-zinc-300'
                          }`}>
                            <div className="truncate font-bold">
                              <span>{l.store}</span>
                              {isLowest && <span className="ml-1 text-[9px] text-emerald-600 dark:text-emerald-400">★ LOWEST</span>}
                            </div>
                            <div className="flex items-center gap-1.5 shrink-0">
                              <span className={`font-black font-mono-num ${isLowest ? 'text-emerald-600 dark:text-emerald-400' : 'text-zinc-800 dark:text-zinc-200'}`}>
                              ...
                              <a
                                ...
                                className="p-1 rounded bg-zinc-200 dark:bg-[#242424] hover:bg-zinc-300 dark:hover:bg-[#333333] text-zinc-800 dark:text-zinc-200"
                              >
              ...
              <div className="grid gap-4 py-3 text-xs font-mono" style={{ gridTemplateColumns: `repeat(${products.length}, minmax(0, 1fr))` }}>
                {products.map((p) => (
                  <div key={p.id}>
                    ...
                    <span className={`text-[10px] uppercase block font-bold mt-0.5 ${p.reviewConfidence === 'high' ? 'text-cyan-600 dark:text-cyan-400' : 'text-zinc-500 dark:text-zinc-400'}`}>
              ...
              <div className="grid gap-4 py-3 text-xs font-mono" style={{ gridTemplateColumns: `repeat(${products.length}, minmax(0, 1fr))` }}>
                {products.map((p) => (
                  <div key={p.id} className="space-y-1">
                    <span className="text-[10px] uppercase font-bold text-emerald-600 dark:text-emerald-400 block flex items-center gap-1">
              ...
              <div className="grid gap-4 py-3 text-xs font-mono" style={{ gridTemplateColumns: `repeat(${products.length}, minmax(0, 1fr))` }}>
                {products.map((p) => (
                  <div key={p.id} className="space-y-1">
                    ...
                    <div className="text-zinc-700 dark:text-zinc-300">FABRIC: <strong className="text-zinc-900 dark:text-white">{p.material || 'N/A'}</strong></div>
                    <div className="text-zinc-700 dark:text-zinc-300">COLOR: <strong className="text-zinc-900 dark:text-white">{p.color || 'N/A'}</strong></div>
                  </div>
                ))}
              </div>
              ...
              <div className="grid gap-4 py-3 text-xs font-mono" style={{ gridTemplateColumns: `repeat(${products.length}, minmax(0, 1fr))` }}>
                {products.map((p) => (
                  <div key={p.id}>
                    ...
                    <div className="font-black text-[#FF3E00] dark:text-[#FF9575]">
                      {p.platformScore} / 10
                    </div>
                  </div>
                ))}
                  ...
                  return (
                    <div key={p.id}>
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="w-full flex items-center justify-center gap-1.5 py-2.5 px-3 rounded-sm bg-[#FF3E00] hover:bg-[#E03600] text-black font-mono font-black text-xs uppercase tracking-wider transition-transform active:scale-95 cursor-pointer"
                      >
