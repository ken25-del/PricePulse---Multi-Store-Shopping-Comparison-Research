...
export const SiteDiscoveryModal: React.FC<SiteDiscoveryModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/60 dark:bg-black/80 backdrop-blur-sm overflow-y-auto animate-in fade-in duration-200">
      <div className="relative w-full max-w-2xl flex flex-col bg-white dark:bg-[#0d0d0d] rounded-xl shadow-2xl border border-zinc-200 dark:border-[#262626] overflow-hidden text-zinc-900 dark:text-[#f4f4f4] transition-colors">
            ...
            {isDiscovering ? (
              ...
              discoveredSites.map((site) => {
                ...
                return (
                  <div
                    key={site.id}
                    className="flex items-center justify-between p-3.5 rounded-lg border border-zinc-200 dark:border-[#262626] hover:border-[#FF3E00]/60 transition-colors bg-zinc-50 dark:bg-[#121212]"
                  >
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        <span className="font-bold text-sm text-zinc-900 dark:text-white">{site.name}</span>
                        <span className="text-[10px] text-zinc-500 font-mono">({site.domain})</span>
                      </div>
                      <p className="text-zinc-600 dark:text-zinc-400 text-xs font-sans">{site.description}</p>
                      {site.categorySpecialty && (
                        <div className="flex gap-1 pt-0.5">
                          {site.categorySpecialty.map((s, i) => (
                            <span key={i} className="px-1.5 py-0.5 rounded-xs bg-zinc-200/70 dark:bg-[#1a1a1a] border border-zinc-300 dark:border-[#2a2a2a] text-zinc-700 dark:text-zinc-300 font-bold uppercase text-[10px]">
                              {s}
                            </span>
                          ))}
                        </div>
                    ...
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-sm text-xs font-bold font-mono uppercase tracking-wider flex items-center gap-1 transition-all cursor-pointer ${
                        isEnabled
                          ? 'bg-emerald-100 text-emerald-800 border border-emerald-300 dark:bg-[#152518] dark:text-emerald-300 dark:border-emerald-800'
        ...
        <div className="px-5 py-3 border-t border-zinc-200 dark:border-[#242424] bg-zinc-50 dark:bg-[#090909] flex justify-end font-mono">
          <button
            onClick={onClose}
            className="px-4 py-2 rounded-sm bg-zinc-200 hover:bg-zinc-300 dark:bg-[#1e1e1e] dark:hover:bg-[#2a2a2a] border border-zinc-300 dark:border-[#333333] font-bold text-xs uppercase tracking-wider text-zinc-900 dark:text-white cursor-pointer"
          >
            DONE
          </button>
        </div>
