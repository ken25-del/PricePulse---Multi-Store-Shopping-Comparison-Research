  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/80 backdrop-blur-sm overflow-y-auto animate-in fade-in duration-200">
      <div className="relative w-full max-w-2xl max-h-[90vh] flex flex-col bg-[#0d0d0d] rounded-xl shadow-2xl border border-[#262626] overflow-hidden text-[#f4f4f4]">
        ...
        {/* Tab Navigation */}
        <div className="flex border-b border-[#242424] px-5 text-xs font-mono font-bold uppercase tracking-wider overflow-x-auto bg-[#0a0a0a]">
          {[
        ...
        <div className="p-5 sm:p-6 overflow-y-auto space-y-6 flex-1 text-xs">
          ...
          {activeTab === 'general' && (
            <div className="space-y-4">
              <div>
                <label className="font-mono font-bold uppercase tracking-wider text-zinc-300 block mb-1.5">
                  LANGUAGE (भाषा)
                </label>
                  ...
                  <button
                    ...
                    className={`p-2.5 rounded-md border text-center font-mono font-bold text-xs uppercase tracking-wider transition-all cursor-pointer ${
                      settings.language === 'en'
                        ...
                        : 'border-[#262626] bg-[#121212] text-zinc-400 hover:text-white hover:border-[#383838]'
                    }`}
                  >
                    English (Default)
                  </button>
              ...
              <div className="pt-2">
                <label className="flex items-center justify-between cursor-pointer p-3 rounded-lg border border-[#262626] bg-[#121212] hover:border-[#383838]">
                  <div>
                    <span className="font-mono font-bold uppercase text-white block">AUTO-DISCOVER SPECIALTY STORES</span>
                    <span className="text-zinc-400 text-[11px]">Suggests verified niche stores (e.g. Nalli, Karagiri, Rare Rabbit)</span>
                  </div>
                  <input
                    type="checkbox"
          ...
          {activeTab === 'ai' && (
            <div className="space-y-4">
              <div className="p-3.5 rounded-lg bg-[#141414] border border-[#2a2a2a] flex items-start gap-2.5">
                <Sparkles className="w-4 h-4 text-[#FF3E00] shrink-0 mt-0.5" />
                <div>
                  <h4 className="font-mono font-bold uppercase tracking-wider text-white text-xs">
                    AI IS 100% OPTIONAL
                  </h4>
              ...
              <label className="flex items-center justify-between cursor-pointer p-3 rounded-lg border border-[#262626] bg-[#121212] hover:border-[#383838]">
                <div>
                  <span className="font-mono font-bold uppercase text-white block">ENABLE AI INTELLIGENCE LAYER</span>
                  <span className="text-zinc-400 text-[11px]">Enables review synthesis & "Should I Buy This?" analysis</span>
                </div>
                <input
                  type="checkbox"
              ...
              {settings.aiEnabled && (
                <div className="space-y-3 pt-2">
                  <div>
                    <label className="font-mono font-bold uppercase tracking-wider text-zinc-300 block mb-1">
                      AI MODEL PROVIDER
                    </label>
                    <select
                      value={settings.aiProvider}
                      onChange={(e) => onUpdateSettings({ aiProvider: e.target.value as any })}
                      className="w-full p-2.5 rounded-md border border-[#2a2a2a] bg-[#121212] text-white font-mono text-xs focus:outline-hidden focus:border-[#FF3E00]"
                    >
                  ...
                  <div>
                    ...
                    {apiKeyInput && (
                      <button
                        ...
                        className="mt-1.5 text-[11px] text-rose-400 hover:underline font-mono font-bold uppercase tracking-wider cursor-pointer"
                      >
                        Clear Stored Key
                      </button>
                    )}
                    ...
                    {keyTestResult && (
                      <div className={`mt-2 p-2.5 rounded-md text-xs font-mono flex items-center gap-2 ${
                        keyTestResult.success
                          ? 'bg-[#102015] text-emerald-300 border border-emerald-800'
                          : 'bg-[#201010] text-rose-300 border border-rose-800'
                      }`}>
                        {keyTestResult.success ? <CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0" /> : <AlertCircle className="w-4 h-4 text-rose-400 shrink-0" />}
          ...
          {activeTab === 'privacy' && (
            <div className="space-y-4">
              <div className="p-3.5 rounded-lg bg-[#141414] border border-[#262626]">
                <h4 className="font-mono font-bold uppercase tracking-wider text-white text-xs mb-1">
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between p-3 rounded-lg border border-[#262626] bg-[#121212]">
                  <div>
                    <span className="font-mono font-bold uppercase text-white block">CLEAR SEARCH HISTORY</span>
                    <span className="text-zinc-400 text-[11px]">Removes recent search suggestions</span>
                  </div>
                  <button
                    onClick={onClearHistory}
                    className="px-3 py-1.5 rounded-sm border border-[#333333] hover:bg-[#1e1e1e] font-mono font-bold text-xs uppercase tracking-wider text-zinc-300 cursor-pointer"
                  >
                    Clear History
                  </button>
                </div>
                ...
                <div className="flex items-center justify-between p-3 rounded-lg border border-[#262626] bg-[#121212]">
                  <div>
                    <span className="font-mono font-bold uppercase text-white block">CLEAR SAVED WISHLIST</span>
                    <span className="text-zinc-400 text-[11px]">Removes all bookmarked product comparisons</span>
                  </div>
                  <button
                    onClick={onClearWishlist}
                    className="px-3 py-1.5 rounded-sm border border-[#401515] hover:bg-[#251010] text-rose-400 font-mono font-bold text-xs uppercase tracking-wider cursor-pointer"
                  >
                    Clear Wishlist
                  </button>
                </div>

                <div className="flex items-center justify-between p-3 rounded-lg border border-rose-900/60 bg-[#201010]">
                  <div>
                    <span className="font-mono font-bold uppercase text-rose-200 block">RESET ALL APPLICATION DATA</span>
                    <span className="text-rose-400 text-[11px]">Purges all local preferences, keys, and cached data</span>
                  </div>
                  <button
                    onClick={() => {
          ...
          {activeTab === 'sources' && (
            <div className="space-y-3">
              ...
              {customSources.length === 0 ? (
                <div className="p-8 text-center text-zinc-500 border border-dashed border-[#2a2a2a] rounded-lg font-mono">
                  <Database className="w-8 h-8 mx-auto text-zinc-600 mb-2" />
                  <p className="font-bold text-zinc-400 uppercase tracking-wider">No custom stores added yet.</p>
                ...
                <div className="space-y-2">
                  {customSources.map((s) => (
                    <div
                      key={s.id}
                      className="flex items-center justify-between p-3 rounded-lg border border-[#262626] bg-[#121212]"
                    >
                      <div>
                        <div className="font-bold text-white tracking-tight">{s.name}</div>
                        <div className="text-[11px] text-zinc-400 font-mono">{s.domain}</div>
                      </div>
                      <button
          ...
          {/* About Tab */}
          {activeTab === 'about' && (
            <div className="space-y-3 leading-relaxed text-zinc-300 font-sans">
              <div className="p-3.5 rounded-lg bg-[#141414] border border-[#262626] text-xs">
                <strong className="font-mono font-bold uppercase tracking-wider text-white block mb-1">
