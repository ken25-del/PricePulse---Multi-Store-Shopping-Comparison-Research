...
export const WishlistModal: React.FC<WishlistModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/60 dark:bg-black/80 backdrop-blur-sm overflow-y-auto animate-in fade-in duration-200">
      <div className="relative w-full max-w-3xl max-h-[90vh] flex flex-col bg-white dark:bg-[#0d0d0d] rounded-xl shadow-2xl border border-zinc-200 dark:border-[#262626] overflow-hidden text-zinc-900 dark:text-[#f4f4f4] my-auto transition-colors">
            ...
            {wishlist.length > 0 && (
              <>
                <button
                  ...
                  className="p-1.5 px-2.5 rounded-sm border border-zinc-300 dark:border-[#303030] hover:bg-zinc-100 dark:hover:bg-[#1a1a1a] text-zinc-700 dark:text-zinc-300 text-xs flex items-center gap-1 font-mono font-bold uppercase tracking-wider cursor-pointer"
                >
                ...
                <button
                  ...
                  className="p-1.5 px-2.5 rounded-sm border border-rose-300 dark:border-[#401515] hover:bg-rose-50 dark:hover:bg-[#251010] text-rose-600 dark:text-rose-400 text-xs flex items-center gap-1 font-mono font-bold uppercase tracking-wider cursor-pointer"
                >
        ...
        <div className="px-5 py-2.5 bg-orange-50/60 dark:bg-[#14120f] border-b border-orange-200 dark:border-[#2d2218] flex items-center gap-2 text-xs font-mono text-[#FF3E00] dark:text-[#FF9575]">
        ...
        <div className="p-5 overflow-y-auto space-y-3 flex-1">
          {wishlist.length === 0 ? (
            <div className="py-16 text-center text-zinc-500 font-mono">
              <Heart className="w-12 h-12 stroke-[1.2] mx-auto text-zinc-400 dark:text-zinc-700 mb-3" />
              <p className="font-bold text-sm text-zinc-700 dark:text-zinc-400 uppercase tracking-wider">Your saved research list is empty.</p>
            ...
            wishlist.map((item) => {
              ...
              return (
                <div
                  key={item.id}
                  className="flex flex-col sm:flex-row items-center justify-between gap-4 p-3.5 rounded-lg border border-zinc-200 dark:border-[#222222] hover:border-[#FF3E00]/60 transition-colors bg-zinc-50 dark:bg-[#121212]"
                >
                  <div className="flex items-center gap-3.5 w-full sm:w-auto">
                    <img
                      ...
                      className="w-16 h-16 rounded-md object-cover bg-zinc-100 dark:bg-[#1a1a1a] shrink-0 border border-zinc-200 dark:border-[#2a2a2a]"
                      onError={(e) => {
                    ...
                    <div className="min-w-0">
                      <h4
                        ...
                        className="font-bold text-xs sm:text-sm text-zinc-900 dark:text-white hover:text-[#FF3E00] cursor-pointer line-clamp-1 tracking-tight"
                      >
                        {item.canonicalTitle}
                      </h4>
                      <div className="flex flex-wrap items-center gap-2 mt-1 text-xs text-zinc-600 dark:text-zinc-400 font-mono">
                  ...
                  <div className="flex items-center gap-2 w-full sm:w-auto justify-end font-mono">
                    <a
                      ...
                      className="flex items-center gap-1 px-3 py-1.5 rounded-sm bg-[#FF3E00] hover:bg-[#E03600] text-black font-mono font-bold text-xs uppercase tracking-wider cursor-pointer"
                    >
        ...
        {/* Footer */}
        <div className="px-5 py-3 border-t border-zinc-200 dark:border-[#242424] bg-zinc-50 dark:bg-[#090909] text-xs text-zinc-500 font-mono flex items-center justify-between transition-colors">
          <span>SAVED LOCALLY IN BROWSER STORAGE. NO SIGN-IN NEEDED.</span>
          <button
            type="button"
            onClick={onClose}
            className="px-3.5 py-1.5 rounded-sm bg-zinc-200 dark:bg-[#1e1e1e] hover:bg-zinc-300 dark:hover:bg-[#2a2a2a] border border-zinc-300 dark:border-[#333333] font-mono font-bold uppercase tracking-wider text-zinc-900 dark:text-white text-xs cursor-pointer"
          >
            Close
          </button>
        </div>
