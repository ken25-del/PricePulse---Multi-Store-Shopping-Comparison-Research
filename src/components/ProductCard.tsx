...
export const ProductCard: React.FC<ProductCardProps> = ({
  ...
  return (
    <div className={`bg-[#111111] border rounded-xl p-4 sm:p-5 transition-all flex flex-col justify-between group shadow-lg ${
      ...
      <div>
        ...
        {currentWinner && (
          <div className="flex items-center justify-between gap-2 mb-3 pb-2.5 border-b border-[#222222]">
            ...
            <span className="text-[11px] font-mono text-zinc-400">
              Platform Index: <strong className="text-white">{productGroup.platformScore}/10</strong>
            </span>
          </div>
        )}
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          {/* Product Image */}
          <div className="relative w-full sm:w-44 h-52 sm:h-44 rounded-lg overflow-hidden bg-[#0c0c0c] shrink-0 border border-[#262626]">
            <img
              ...
              <button
                ...
                className={`p-1.5 rounded-md backdrop-blur-md transition-colors border cursor-pointer ${
                  isWishlisted
                    ...
                    : 'bg-black/80 text-zinc-300 hover:text-white border-[#333333]'
                }`}
              >
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex flex-wrap items-center gap-1.5 mb-2 font-mono">
              {productGroup.savingsPercent >= 10 && (
                <span className="px-2 py-0.5 rounded-sm text-[10px] font-black uppercase tracking-wider bg-[#1c2e1f] text-emerald-400 border border-[#27532d]">
              ...
              {productGroup.reviewConfidence && (
                <span className={`px-2 py-0.5 rounded-sm text-[10px] font-bold uppercase tracking-wider border ${
                  productGroup.reviewConfidence === 'high' 
                    ? 'bg-[#102024] text-cyan-400 border-[#1c444e]'
                    : 'bg-[#1a1a1a] text-zinc-400 border-[#2e2e2e]'
                }`}>
                  {productGroup.reviewConfidence} Review Confidence
                </span>
              )}
              {productGroup.brand && (
                <span className="px-2 py-0.5 rounded-sm text-[10px] font-bold uppercase tracking-wider bg-[#1c1c1c] text-zinc-300 border border-[#2e2e2e]">
                  {productGroup.brand}
                </span>
              )}
            </div>
            ...
            <h3
              ...
              className="font-bold text-base sm:text-lg text-white line-clamp-2 hover:text-[#FF3E00] cursor-pointer tracking-tight leading-snug"
            >
              {productGroup.canonicalTitle}
            </h3>

            {/* Attributes & Material */}
            <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-zinc-400 mt-2 font-mono">
              {productGroup.material && (
                <span>FABRIC: <strong className="text-zinc-200">{productGroup.material}</strong></span>
              )}
              {productGroup.color && (
                <span>COLOR: <strong className="text-zinc-200">{productGroup.color}</strong></span>
              )}
              {productGroup.occasion && (
                <span>OCCASION: <strong className="text-zinc-200">{productGroup.occasion}</strong></span>
              )}
            </div>

            {/* Ratings & Price vs Quality Snapshot */}
            <div className="flex flex-wrap items-center gap-2.5 mt-2.5 font-mono">
              <div className="flex items-center gap-1 bg-[#1a1410] text-[#FF9575] px-2 py-0.5 rounded-sm font-black text-xs border border-[#442217]">
              ...
              {productGroup.priceVsQuality && (
                <span className="text-[11px] text-zinc-400 bg-[#161616] px-2 py-0.5 rounded border border-[#292929]">
                  Value Rating: <strong className="text-emerald-400">{productGroup.priceVsQuality.valueRating}</strong>
                </span>
              )}

              {productGroup.sentiment && productGroup.sentiment !== 'insufficient_data' && (
                <span className="text-[11px] text-zinc-400 bg-[#161616] px-2 py-0.5 rounded border border-[#292929]">
                  Sentiment: <strong className={productGroup.sentiment === 'positive' ? 'text-emerald-400' : 'text-amber-400'}>{productGroup.sentiment.toUpperCase()}</strong>
                </span>
              )}
            </div>
          </div>
        ...
        <div className="mt-3.5 space-y-2">
          ...
          {productGroup.whyRecommended && (
            <div className="p-3 rounded-lg bg-[#141814] border border-[#223d26] text-xs">
              <div className="flex items-center gap-1.5 font-mono font-bold uppercase tracking-wider text-emerald-400 text-[11px] mb-1">
          ...
          {productGroup.thingsToConsider && (
            <div className="p-3 rounded-lg bg-[#181612] border border-[#423219] text-xs">
              <div className="flex items-center gap-1.5 font-mono font-bold uppercase tracking-wider text-amber-400 text-[11px] mb-1">
          ...
          {productGroup.whyNotCheapestExplanation && (
            <div className="p-2.5 rounded-lg bg-[#1a1410] border border-[#442217] text-xs">
              <span className="font-mono text-[10px] font-black uppercase tracking-wider text-[#FF9575] block mb-0.5">
        ...
        <div className="mt-3">
          <button
            ...
            className="w-full flex items-center justify-between px-3 py-1.5 rounded-lg bg-[#151515] hover:bg-[#1c1c1c] text-zinc-300 text-xs font-mono border border-[#262626] transition-colors cursor-pointer"
          >
          ...
          {showFullResearch && (
            <div className="mt-2.5 p-3 rounded-xl bg-[#0e0e0e] border border-[#262626] space-y-3">
              {/* Buyers Like vs Dislike */}
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
                <div className="bg-[#121c14] border border-[#204027] p-2.5 rounded-lg">
                  <div className="flex items-center gap-1 text-emerald-400 font-mono font-bold text-[11px] mb-1.5">
                ...
                <div className="bg-[#1c1412] border border-[#442017] p-2.5 rounded-lg">
                  <div className="flex items-center gap-1 text-amber-400 font-mono font-bold text-[11px] mb-1.5">
              ...
              {productGroup.sourceReviewBreakdown && productGroup.sourceReviewBreakdown.length > 0 && (
                <div className="pt-2 border-t border-[#222222]">
                  <span className="font-mono text-[10px] font-bold uppercase text-zinc-400 block mb-1.5">
                    Ratings Breakdown by Store
                  </span>
                  <div className="flex flex-wrap gap-2">
                    {productGroup.sourceReviewBreakdown.map((s, idx) => (
                      <div key={idx} className="bg-[#161616] border border-[#2c2c2c] px-2 py-1 rounded text-[11px] font-mono flex items-center gap-1.5">
                        <span className="text-zinc-300 font-bold">{s.store}:</span>
                        <span className="text-[#FF9575] font-bold">{s.rating}★</span>
        ...
        <div className="mt-4 pt-3 border-t border-[#222222]">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1 text-xs font-mono font-bold uppercase tracking-wider mb-2.5">
            ...
            <div className="flex items-center gap-1.5 text-[11px]">
              <span className="text-zinc-400 font-normal">Lowest:</span>
              ...
              return (
                <div
                  ...
                  className={`flex flex-col sm:flex-row sm:items-center justify-between p-2.5 rounded-lg text-xs transition-colors border gap-2 ${
                    isLowest
                      ? 'bg-[#0f1f14] border-[#1d4c26] text-white shadow-sm'
                      : 'bg-[#151515] border-[#242424] hover:bg-[#1a1a1a]'
                  }`}
                >
                  ...
                  <div className="flex items-center gap-2 min-w-0 font-mono">
                    ...
                    {isLowest ? (
                      <span className="px-2 py-0.5 text-[9px] font-black uppercase tracking-wider rounded-xs bg-emerald-400 text-black">
                        LOWEST PRICE
                      </span>
                    ) : diffFromLowest > 0 ? (
                      <span className="px-1.5 py-0.5 text-[9px] font-bold rounded-xs bg-[#241a18] text-amber-300 border border-[#442720]">
      ...
      <div className="mt-4 pt-3 border-t border-[#222222] flex items-center justify-between">
        <div className="text-[11px] font-mono text-zinc-400">
          RANGE: <strong className="text-white font-mono-num">₹{productGroup.minPrice.toLocaleString('en-IN')}</strong> — <strong className="text-white font-mono-num">₹{productGroup.maxPrice.toLocaleString('en-IN')}</strong>
        </div>
